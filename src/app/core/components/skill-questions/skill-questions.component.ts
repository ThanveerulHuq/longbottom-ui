...
export class SkillQuestionsComponent implements OnInit {
  ...
  submitAnswers() {
    const isAnyUnanswered = this.questions.some(({ option }) => option !== undefined);
    if (!isAnyUnanswered) {
      ...
      this._snackBar.open('Please answer all the questions to submit', '', { horizontalPosition: "right", verticalPosition: "top", duration: 2000 })
    }
  }
